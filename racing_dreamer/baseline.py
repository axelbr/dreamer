...
class GapFollower:
    ...
    def action(self, observation: Dict[str, np.ndarray]) -> Dict:
        ...
        action = {
            'motor': (np.random.normal(loc=2.0, scale=0.01), np.random.normal(loc=0.3, scale=0.01)),
            'steering': np.random.normal(loc=angle, scale=0.0)
        }
        return action
