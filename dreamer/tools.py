...
from rlephant import Dataset, Episode, Transition
...
def collect_data(agent: Callable[[Dict], Any], env: gym.Env, dataset: Dataset, steps: int):
    t = 0
    while t < steps:
        done = False
        obs = env.reset()
...
def load_dataset(directory, config, preprocess: Callable):
