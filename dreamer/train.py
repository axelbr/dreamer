...
def configure_agent(config, train_envs: List[gym.Env]) -> RacingDreamer:
...
def log_summary(logs: Dict[str, Any], writer: tf.summary.SummaryWriter):
...
def initialize_dataset(config, train_envs):
    datadir = config.logdir / 'episodes.h5'
    gapfollower = GapFollower()
    dataset = Dataset(filename=datadir)
    print(f'Dataset contains {dataset.transition_count} transitions.')
    if dataset.transition_count < config.prefill:
        steps = config.prefill - dataset.transition_count
        print(f'Prefill dataset with {steps} steps.')
        tools.collect_data(agent=lambda o: gapfollower.action(o), env=train_envs[0], steps=steps, dataset=dataset)
    return dataset
...
def train_dreamer(agent, config, test_envs, train_envs, dataset, writer):
  for logs in agent.train(steps=config.steps, dataset=dataset, env=train_envs[0]):
    log_summary(logs=logs, writer=writer)
...
def main(config):
    ...
    dataset = initialize_dataset(config, train_envs)
    agent = configure_agent(config, train_envs)
    train_dreamer(
        agent=agent,
        config=config,
        test_envs=test_envs,
