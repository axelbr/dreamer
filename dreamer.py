...
def make_test_env(config, writer, datadir, gui=False):
    ...
    callbacks = []
    callbacks.append(
        lambda episodes: summarize_episode(episodes, config, datadir, writer, 'test'))
    env = wrappers.Collect(env, callbacks, config.precision)
    return env
...
def create_log_dirs(config):
    logdir = pathlib.Path(
        f'{config.logdir}/{config.track}_dreamer_{config.task}_{config.obs_type}_{config.action_dist}_Ar{config.action_repeat}_Bl{config.batch_length}_H{config.horizon}_{config.seed}_{time.time()}')
...
def main(config):
    ...
    if config.prefill_agent == 'random':
        ...
        random_agent = lambda o, d, s: (
            [train_env.action_space[id].sample()], None)  # note: it must work as single_agent agent
        ...
        gap_follower_agent = lambda o, d, s: (
            [np.clip(np.array([fixed_speed, gapfollower.action(o)[-1]]), -1, +1)], None)
